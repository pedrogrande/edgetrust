...
import { EventType, TaskState, TaskType, VerificationMethod } from '@/types/trust-builder';
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (groups.length === 0) {
      return new Response(
        JSON.stringify({
          error: 'The selected Mission does not exist or is not active. Please choose a valid Mission.',
        }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    const task = await withTransaction(import.meta.env.DATABASE_URL, async (client) => {
